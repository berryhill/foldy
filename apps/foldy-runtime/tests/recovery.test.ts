...
test('consistent custody-free snapshot restores history and permits fresh authorized publication',()=>{
 ...
 const run=(op:string,extra:any={})=>d.dispatch(op,{projectId:'p',expectedBaseRevisionId:d.current(),idempotencyKey:String(++n),...extra},owner);
 const publish=()=>{let a=run('create_update',{title:'proposal'});let ref={updateId:a.updateId,expectedUpdateRevisionId:a.updateRevisionId};a=run('update_page',{...ref,path:'index.html',content:'changed'+n});ref.expectedUpdateRevisionId=a.updateRevisionId;run('submit_update_for_review',ref);const c=run('add_review_comment',{...ref,text:'fix',blocking:true,target:{path:'index.html'}});run('resolve_review_comment',{...ref,commentId:c.commentId,reason:'fixed'});run('submit_update_for_review',ref);run('approve_update_revision',{...ref,reason:'approved'});return run('publish_update',{...ref,reason:'publish'});};
