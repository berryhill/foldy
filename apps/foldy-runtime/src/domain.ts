...
const reads=['get_project','get_workbook','list_pages','get_page','list_files','get_file','search','list_updates','get_update','get_update_changes','get_update_preview','list_review_comments','get_readiness_checks','get_revision_history','list_update_receipts'];
...
export class Domain {
 ...
 // Backup materializes revisions in ID order, so return the same stable order
 // before and after restore; parent links carry lineage, not row position.
 if(name==='get_revision_history')value=this.db.prepare('SELECT id,parent,actor FROM revisions ORDER BY id').all();
 if(args.updateId){const u=this.update(args.updateId as string);if(args.atRevisionId&&u.revision!==args.atRevisionId)fail('REVISION_CONFLICT');if(name==='get_update')value={...u,decisions:this.db.prepare("SELECT value FROM receipts WHERE json_extract(value,'$.updateId')=? AND json_extract(value,'$.operation') IN ('request_update_changes','close_update','approve_update_revision','publish_update') ORDER BY rowid").all(u.id).map(r=>JSON.parse(r.value as string)).map(r=>({receiptId:r.receiptId,operation:r.operation,reason:r.reason,actorRef:r.actorRef,occurredAt:r.occurredAt,updateRevisionId:r.updateRevisionId,currentState:r.currentState}))};if(name==='list_update_receipts'){
 const limit=args.limit===undefined?50:Number(args.limit);
 if(!Number.isSafeInteger(limit)||limit<1||limit>100||args.limit!==undefined&&String(limit)!==args.limit)fail('REQUEST_INVALID');
 let after=0;
 if(args.afterReceiptId!==undefined){
  const cursor=this.db.prepare("SELECT rowid FROM receipts WHERE json_extract(value,'$.updateId')=? AND json_extract(value,'$.receiptId')=?").get(u.id,args.afterReceiptId as string);
  if(!cursor)fail('REQUEST_INVALID');after=cursor.rowid as number;
 }
 const rows=this.db.prepare("SELECT value FROM receipts WHERE rowid>? AND json_extract(value,'$.updateId')=? ORDER BY rowid LIMIT ?").all(after,u.id,limit+1);
 // Allowlist the durable, non-authored receipt evidence; never return stored inputs,
 // idempotency keys, reasons, authored comments, titles, or revision file bytes.
 const receipts=rows.slice(0,limit).map(row=>{const r=JSON.parse(row.value as string);return {
  projectId:r.projectId,updateId:r.updateId,receiptId:r.receiptId,operation:r.operation,
  priorState:r.priorState,currentState:r.currentState,updateRevisionId:r.updateRevisionId,
  currentPublishedRevisionId:r.currentPublishedRevisionId,occurredAt:r.occurredAt,actorRef:r.actorRef,
  ...(r.commentId?{commentId:r.commentId}:{}),
  ...(r.checksEvidence?{checksEvidence:{revisionId:r.checksEvidence.revisionId,filesDigest:r.checksEvidence.filesDigest,pass:r.checksEvidence.pass,failureCount:r.checksEvidence.failures?.length??0}}:{}),
